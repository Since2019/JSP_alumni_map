...
<%
		...
		ResultSet rs = null;
		Connection con = null;
		String username=request.getParameter("username").trim();//去除首尾空格
		String password=request.getParameter("password").trim();
		String refill=request.getParameter("refill").trim();
		try{
		ctx = new InitialContext();
		ds = (DataSource)ctx.lookup("java:comp/env/jdbc/ClassmateLogin");//ClassmateInfo是在创建连接池时自己创建的名字
		...
		if(username!=null ){
			...
			if(rs.next()){
				...
				if(password.equals(refill)){
	 				stmt.executeUpdate("insert into students(id,password,name,city,school,note,longitude,latitude) values('"+username+"','"+ password + "','"+username+"','"+username+"','"+username+"','"+username+"','"+username+"','"+username+"');");
				%>
				注册成功！！！<br>
				三秒钟后自动转到登录页面！！！<br>
